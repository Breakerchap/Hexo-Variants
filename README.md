...
It includes stackable rule modes like `Triangle Grid`, `Square Grid`, `Octagon Grid`, `Entropy Cascade`, `Armory`, `Duck`, `King Duck`, `Egyptian`, `Echo`, `Orbit`, and `Meteor`.
...
- **Entropy Cascade**: after each full turn, a deterministic multi-phase chaos simulation mutates the board (stone removals, ownership flips, growth bursts, ring interference, and aftershock culls) before winner checks.
- **Armory**: adds player classes, dual currencies, rotating shops, inventory-based piece deployment, and end-turn tactical ability cycles.
